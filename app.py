...
# =========================================
# Rejection Analysis (Streamlit App)
# - Reads uploaded .xlsx
# - Identifies rejected rows (Paid==0 AND status=="rejected" AND DenialCode not empty)
# - Builds the SAME output sheets as your CLI script
# =========================================
...
def ensure_numeric(df: pd.DataFrame) -> pd.DataFrame:
    # same columns used in your script :contentReference[oaicite:3]{index=3}
    num_cols = [
        "ActivityIns",
...
def build_rejected_df(df: pd.DataFrame) -> pd.DataFrame:
    # same rule as your script :contentReference[oaicite:4]{index=4}
...
def build_rejection_workbook_bytes(input_xlsx_bytes: bytes, input_filename: str) -> tuple[bytes, dict]:
...
# -------------------- UI --------------------
st.title("Rejection Analysis (Excel Upload → Excel Output)")

st.caption(
    "Rule used: Paid==0 AND ActivityStatus=='rejected' AND DenialCode not empty "
    "(same as your original script)."
)

uploaded = st.file_uploader("Upload your source .xlsx file", type=["xlsx"])
