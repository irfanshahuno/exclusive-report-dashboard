...
from kpi.kpi_utils import quarter_dates, make_patient_key, clean_result_value, PRIMARY_CARE_DEPTS
...
def compute_pc009(visits_df, labs_df, year, quarter):
    ...
    visits_df["is_diab"] = visits_df.apply(
        lambda r: has_diabetes(r["ICD (Principal)"], r["ICD (Secondary)"]), axis=1
    )
