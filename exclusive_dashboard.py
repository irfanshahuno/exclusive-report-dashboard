...
def resolve_pharmacy_generator() -> Path | None:
    """Return whichever pharmacy generator file exists (Linux is case-sensitive)."""
    candidates = [
...
CENTERS = {
    ...
    "excellent_pharmacy": {
        ...
        "src_name": "source.xlsx",
        "out_name": "report.xlsx",
        "generator": GEN_PHARM,  # may be None -> we show a clear diagnostic below
    },
}
...
def rebuild_report(generator: Path, src_path: Path, out_path: Path) -> str:
    ...
    try:
        ...
        res = _run([py, str(generator), src, "--out", out])  # support old order
        return res.stdout or "OK"
...
generator = cfg["generator"]  # may be None for pharmacy if file not found
    ...
    if colA.button("↻ Rebuild report", use_container_width=True):
        try:
            if generator is None:
                raise RuntimeError("Pharmacy generator file not found. Allowed names:\n"
                                   " - pharmacy_exclusive_report_with_aging.py\n"
                                   " - pharmacy_exclusive_report_with_Aging.py\n"
                                   " - pharmacy_exclusive_report_with_aging_final.py")
...
try:
    ...
    def add_grand_total_if_missing(df: pd.DataFrame) -> pd.DataFrame:
        ...
        if df[ins_col].astype(str).str.contains("grand total", case=False, na=False).any():
            return df
        # choose numeric columns for sum
        num_cols = [c for c in df.columns if pd.api.types.is_numeric_dtype(df[c])]
        sums = {c: float(df[c].sum()) for c in num_cols}
        gt = {col: "" for col in df.columns}
        gt.update(sums)
        gt[ins_col] = "Grand Total"
        df2 = pd.concat([df, pd.DataFrame([gt], columns=df.columns)], ignore_index=True)
        return df2
    ...
    def kpi_vals(totals_df: pd.DataFrame):
        ...
        if row is not None:
            ...
            # tolerate both "NetAmount" and "Net Amount"
