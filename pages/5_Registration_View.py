...
def _dfs_to_html(dfs: dict, title: str, picked_label: str) -> str:
    ...
    def _kpi_card(label, val):
        return (
            ...
            f"border:1.5px solid #e2eaf5;border-radius:14px;padding:18px 22px;"
            f"box-shadow:0 2px 12px rgba(10,38,71,0.07),0 1px 3px rgba(10,38,71,0.04);'>"
            f"<div style='color:#7B90B2;font-size:11px;font-weight:700;"
            f"text-transform:uppercase;letter-spacing:0.8px;margin-bottom:10px;'>{label}</div>"
            f"<div style='color:#0B2342;font-size:30px;font-weight:900;"
            f"letter-spacing:-1px;line-height:1;'>{val}</div>"
            f"</div></td>"
        )
...
<div style="max-width:960px;margin:24px auto;border-radius:18px;
     box-shadow:0 16px 48px rgba(10,38,71,0.18);overflow:hidden;">

  <!-- ── Dark header ── -->
  <div style="background:#0B2342;padding:22px 28px;">
    <div style="color:#ffffff;font-size:20px;font-weight:900;letter-spacing:-0.04em;">
      📌 EMC Income Analysis Report
    </div>
    <div style="color:#7fa8d4;font-size:12px;margin-top:6px;font-weight:500;">
      {picked_label} &nbsp;·&nbsp; Generated: {pd.Timestamp.now().strftime('%d %b %Y %H:%M')}
    </div>
  </div>

  <!-- ── KPI section ── -->
  <div style="background:#EDF2FB;padding:20px 22px 4px 22px;">
  ...
  <div style="background:#ffffff;padding:20px 22px;">
    ...
    <div style="background:#0B2342;border-radius:10px;padding:12px 18px;margin-bottom:18px;">
      <span style="color:#ffffff;font-size:14px;font-weight:800;letter-spacing:-0.2px;">
