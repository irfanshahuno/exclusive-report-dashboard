...
def render_summary(dfs: Dict[str, pd.DataFrame], day_ts: pd.Timestamp):
    ...
    # Extra: Bulk month filter (optional)
    if bulk_mode:
        st.markdown("**Bulk options**")
        st.checkbox(
            "Process only a selected month (instead of all dates found in Registration)",
            value=False,
            key="bulk_month_only",
            help="Useful when you upload a full-year Registration file but want to save only one month to S3.",
        )
        if st.session_state.get("bulk_month_only"):
            # Try to infer available years from the Registration file date column
            _days = get_days_from_registration(SS.get("reg_df")) if SS.get("reg_df") is not None else []
            _years = sorted({pd.to_datetime(d).year for d in _days}) if _days else [pd.to_datetime(manual_day).year]
            st.selectbox("Select Year (Bulk)", options=_years, index=len(_years)-1, key="bulk_year")

            months = list(range(1, 13))
            month_labels = [datetime(2000, m, 1).strftime("%B") for m in months]
            default_label = datetime(2000, pd.to_datetime(manual_day).month, 1).strftime("%B")
            picked_label = st.selectbox("Select Month (Bulk)", options=month_labels, index=month_labels.index(default_label), key="bulk_month_label")
            # store numeric month for processing
            st.session_state["bulk_month"] = month_labels.index(picked_label) + 1
    ...
    if st.button(process_label, type="primary", disabled=not can_process):
        ...
        if bulk_mode:
            ...
            # Optional month filter for bulk
            if st.session_state.get("bulk_month_only"):
                _y = st.session_state.get("bulk_year")
                _m = st.session_state.get("bulk_month")
                if _y is not None and _m is not None:
                    try:
                        _y = int(_y); _m = int(_m)
                        days = [d for d in days if pd.to_datetime(d).year == _y and pd.to_datetime(d).month == _m]
                    except Exception:
                        pass
