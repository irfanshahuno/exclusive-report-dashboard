...
def run_summary_engine(uploaded_bytes: bytes, filename: str) -> dict:
    ...
    if "Status" not in df.columns:
        df["Status"] = ""
    # Forward-fill Status — source Excel has merged cells, blank rows get NaN
    df["Status"] = df["Status"].replace("", pd.NA).ffill().fillna("")

    # ── Compute measures ─────────────────────────────────────────────────────
    df["Net Amount"] = df["SubInsShare"]

    # ── Date column for aging ─────────────────────────────────────────────────
    ...
    df["Paid"] = (remit_sum - df["TakeBack"].abs()).clip(lower=0)
    ...
    # 1. Final Rejection → Rejected
    df.loc[mask_rej, "Rejected"]  = df.loc[mask_rej, "Residual"]
    df.loc[mask_rej, "Residual"]  = 0.0

    # 2. Rejection Accepted → Accepted
...
def build_insurance_aging_excel(df_detail: pd.DataFrame, df_source: pd.DataFrame, date_range: str = "") -> bytes:
    ...
    # ── Merge extra cols from source ──────────────────────────────────────────
    EXTRA = ["SubDate", "InsGroup", "InsPlan", "MemberID", "EncPatID", "EncType",
             "EncStart", "EncEnd", "DepName", "ReceiverID"]
    if "UniqueID" in df_source.columns and "UniqueID" in d.columns:
    ...
    for i, col in enumerate(final_cols, 1):
        ...
        ws.column_dimensions[ltr].width = 30 if col == "Insurance" else (18 if col in TEXT_COLS else 16)
...
def send_rcm_email(excel_bytes: bytes, excel_filename: str, result: dict, center_name: str) -> tuple:
    ...
    def _df_to_html(df, caption=""):
        ...
        ths = "".join(
            ...
            f"font-size:10px;white-space:nowrap;"
        ...
        for ri, (_, row) in enumerate(df.iterrows()):
            ...
            for ci, val in enumerate(row):
                ...
                cells += (
                    ...
                    f"font-size:10px;font-family:Calibri,Arial,sans-serif;'>{val}</td>"
                )
            rows_html += f"<tr>{cells}</tr>"

        return (
            f"<table style='border-collapse:collapse;width:100%;"
            f"font-size:10px;font-family:Calibri,Arial,sans-serif;margin-bottom:20px;'>"
