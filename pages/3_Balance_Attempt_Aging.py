...
def build_stage_aging_summary(balance_df: pd.DataFrame) -> pd.DataFrame:
    b = balance_df.copy()
    b["Balance"] = pd.to_numeric(b["Balance"], errors="coerce").fillna(0)
    b["AgingBucket"] = b.get("AgingBucket", "Unknown").astype(str)
    ...
    # For Not Submitted (no stage date), do not allocate to aging buckets
    b.loc[b["SubmissionStage"] == "Not Submitted", "AgingBucket"] = "No Stage Date"

    piv = pd.pivot_table(
        b,
        index="SubmissionStage",
        columns="AgingBucket",
        values="Balance",
        aggfunc="sum",
        fill_value=0,
    ...
    for col in aging_order:
        if col not in piv.columns:
            piv[col] = 0.0
    if "No Stage Date" not in piv.columns:
        piv["No Stage Date"] = 0.0

    piv["Total"] = piv[aging_order + ["No Stage Date"]].sum(axis=1)

    # ensure all main rows exist
    existing = set(piv["SubmissionStage"].astype(str))
    missing_rows = []
    for stg in stage_order:
        if stg not in existing:
            row = {"SubmissionStage": stg}
            for c in aging_order + ["No Stage Date", "Total"]:
                row[c] = 0.0
            missing_rows.append(row)
    if missing_rows:
        piv = pd.concat([piv, pd.DataFrame(missing_rows)], ignore_index=True)

    piv["SubmissionStage"] = pd.Categorical(piv["SubmissionStage"], categories=stage_order, ordered=True)
    piv = piv.sort_values("SubmissionStage").reset_index(drop=True)
    piv["SubmissionStage"] = piv["SubmissionStage"].astype(str)

    grand = pd.DataFrame([{
        "SubmissionStage": "Grand Total",
        **{c: float(piv[c].sum()) for c in aging_order + ["No Stage Date", "Total"]}
    }])

    return pd.concat([piv, grand], ignore_index=True)


def build_stage_over60_kpis(balance_df: pd.DataFrame) -> dict:
    b = balance_df.copy()
    b["Balance"] = pd.to_numeric(b["Balance"], errors="coerce").fillna(0)
    over60 = is_over_60_bucket(b["AgingBucket"])

    def stage_sum(stage_name: str) -> float:
        return float(b.loc[(b["SubmissionStage"] == stage_name) & over60, "Balance"].sum())

    initial = stage_sum("Initial Submission")
    r1 = stage_sum("Resubmission 1")
    r2 = stage_sum("Resubmission 2")
    r3 = stage_sum("Resubmission 3")

    return {
        "Initial >60 Days": initial,
        "Resubmission 1 >60 Days": r1,
        "Resubmission 2 >60 Days": r2,
        "Resubmission 3 >60 Days": r3,
        "Total >60 Days": initial + r1 + r2 + r3,
    }


def build_insurance_stage_aging_detail(balance_df: pd.DataFrame, selected_stage: str) -> pd.DataFrame:
    b = balance_df.copy()
    if "Insurance" not in b.columns:
        b["Insurance"] = "Not Available"
    ...
    if selected_stage == "Not Submitted":
        filtered = b[b["SubmissionStage"] == selected_stage].copy()
        if filtered.empty:
            return pd.DataFrame(columns=["Insurance", "No Stage Date", "Total"])
        grouped = filtered.groupby("Insurance", as_index=False)["Balance"].sum()
        grouped.rename(columns={"Balance": "No Stage Date"}, inplace=True)
        grouped["Total"] = grouped["No Stage Date"]
        grouped = grouped.sort_values("Total", ascending=False).reset_index(drop=True)
        grand = pd.DataFrame([{
            "Insurance": "Grand Total",
            "No Stage Date": float(grouped["No Stage Date"].sum()),
            "Total": float(grouped["Total"].sum()),
        }])
        return pd.concat([grouped, grand], ignore_index=True)

    aging_order = ["0–30 Days", "31–45 Days", "46–60 Days", "61–90 Days", "91–120 Days", ">120 Days"]

    filtered = b[(b["SubmissionStage"] == selected_stage) & (b["AgingBucket"].isin(aging_order))].copy()
    piv = pd.pivot_table(
        filtered,
        index="Insurance",
        columns="AgingBucket",
    ...
    if piv.empty:
        piv = pd.DataFrame(columns=["Insurance"] + aging_order)

    piv["Total"] = piv[aging_order].sum(axis=1)
    piv = piv[["Insurance"] + aging_order + ["Total"]]
    piv = piv.sort_values("Total", ascending=False).reset_index(drop=True)

    grand = pd.DataFrame([{
        "Insurance": "Grand Total",
        **{c: float(piv[c].sum()) for c in aging_order + ["Total"]}
    }])

    return pd.concat([piv, grand], ignore_index=True)


def render_stage_over60_kpis(stage_over60_kpis: dict):
    def fmt(x):
        try:
            return f"{float(x):,.2f}"
        except Exception:
    ...
    <div class="kpi-grid">
      <div class="kpi-card">
        <div class="kpi-label">Initial &gt;60 Days</div>
        <div class="kpi-value">{fmt(stage_over60_kpis.get('Initial >60 Days', 0))}</div>
      </div>

      <div class="kpi-card">
        <div class="kpi-label">Resubmission 1 &gt;60 Days</div>
        <div class="kpi-value">{fmt(stage_over60_kpis.get('Resubmission 1 >60 Days', 0))}</div>
      </div>

      <div class="kpi-card">
        <div class="kpi-label">Resubmission 2 &gt;60 Days</div>
        <div class="kpi-value">{fmt(stage_over60_kpis.get('Resubmission 2 >60 Days', 0))}</div>
      </div>

      <div class="kpi-card">
        <div class="kpi-label">Resubmission 3 &gt;60 Days</div>
        <div class="kpi-value">{fmt(stage_over60_kpis.get('Resubmission 3 >60 Days', 0))}</div>
      </div>

      <div class="kpi-card current">
        <div class="kpi-label">Total &gt;60 Days</div>
        <div class="kpi-value">{fmt(stage_over60_kpis.get('Total >60 Days', 0))}</div>
      </div>
    </div>
    """
    st.markdown(html, unsafe_allow_html=True)
...
def load_kpis_only(path_str: str, token: float, center_key: str):
    ...
    stage_over60_kpis = build_stage_over60_kpis(balance_df)
    insurance_stage_summary = build_insurance_stage_summary(balance_df)
    stage_aging_summary = build_stage_aging_summary(balance_df)
...
def render_center_kpis_only(center_key: str, year: int):
    ...
    render_stage_over60_kpis(stage_over60_kpis)
    st.caption(f"Sold-to-Klaim keywords: {', '.join(keywords_used)}")

    st.subheader("Stage-wise Aging Summary")
    stage_aging_show_df = stage_aging_summary.copy()
    stage_aging_numeric_cols = ["0–30 Days", "31–45 Days", "46–60 Days", "61–90 Days", "91–120 Days", ">120 Days", "No Stage Date", "Total"]
    for c in stage_aging_numeric_cols:
        if c in stage_aging_show_df.columns:
            stage_aging_show_df[c] = pd.to_numeric(stage_aging_show_df[c], errors="coerce").fillna(0.0)

    st.dataframe(stage_aging_show_df, use_container_width=True, hide_index=True)

    stage_aging_buffer = io.BytesIO()
    with pd.ExcelWriter(stage_aging_buffer, engine="openpyxl") as writer:
        stage_aging_show_df.to_excel(writer, index=False, sheet_name="Stage_Aging_Summary")
    stage_aging_buffer.seek(0)

    st.download_button(
        label="📥 Download Stage-wise Aging Summary",
        data=stage_aging_buffer.getvalue(),
        file_name=f"{center_key}_{year}_stage_aging_summary.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        use_container_width=True,
        key=f"download_stage_aging_summary_{center_key}_{year}",
    )
    ...
    st.subheader("Insurance-wise Aging Detail")
    stage_options = ["Initial Submission", "Resubmission 1", "Resubmission 2", "Resubmission 3", "Not Submitted"]
    selected_stage = st.selectbox(
        "Select Submission Stage",
        stage_options,
        index=0,
        key=f"stage_filter_{center_key}_{year}",
    )

    insurance_stage_aging_df = build_insurance_stage_aging_detail(balance_df, selected_stage).copy()
    aging_detail_numeric_cols = ["0–30 Days", "31–45 Days", "46–60 Days", "61–90 Days", "91–120 Days", ">120 Days", "No Stage Date", "Total"]
    for c in aging_detail_numeric_cols:
        if c in insurance_stage_aging_df.columns:
            insurance_stage_aging_df[c] = pd.to_numeric(insurance_stage_aging_df[c], errors="coerce").fillna(0.0)

    st.dataframe(insurance_stage_aging_df, use_container_width=True, hide_index=True)

    aging_detail_buffer = io.BytesIO()
    with pd.ExcelWriter(aging_detail_buffer, engine="openpyxl") as writer:
        insurance_stage_aging_df.to_excel(writer, index=False, sheet_name="Insurance_Aging_Detail")
    aging_detail_buffer.seek(0)

    safe_stage = selected_stage.lower().replace(" ", "_").replace(">", "gt").replace("-", "_")
    st.download_button(
        label=f"📥 Download {selected_stage} Aging Detail",
        data=aging_detail_buffer.getvalue(),
        file_name=f"{center_key}_{year}_{safe_stage}_aging_detail.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        use_container_width=True,
        key=f"download_stage_aging_detail_{center_key}_{year}",
    )
