...
def compute_measures(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Paid = sum of all remit + takeback columns
    paid_cols = [
        "RemitInsShare",
        "Resub1RemitInsShare",
        "Resub2RemitInsShare",
    ...
    # Status-based classification using "Status" column only
    df["Rejection"] = 0.0
    df["Accepted"]  = 0.0
    ...
    if "Status" in df.columns:
        ...
        # Accepted: status contains "Rejection Accepted" (check this FIRST)
...
def add_aging(df: pd.DataFrame) -> pd.DataFrame:
    # Always use SubDate for aging
    if "SubDate" in df.columns:
        df["SubDate"] = pd.to_datetime(df["SubDate"], errors="coerce", dayfirst=True)
        df["RefDate"] = df["SubDate"]
    else:
        df["RefDate"] = pd.NaT
...
def apply_styling(output_file: str):
    ...
    for ws in wb.worksheets:
        ...
        # Grand Total column styling for aging summary
