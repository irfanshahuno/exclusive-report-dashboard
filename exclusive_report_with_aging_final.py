...
# and writes Exclusive_Report_with_Aging.xlsx (original behavior).
# ============================================================

# ---- Tuning knobs (kept small & safe) ----
...
def main():
    ...
    denial_code_col = "DenialCode" if "DenialCode" in df.columns else None

    lower_status = df[status_col].astype(str).str.lower() if status_col else pd.Series("", index=df.index)

    # Consider both 'rejected' and 'denied' as denial states (do not rely on DenialCode existing)
    ...
    money_cols = ["Paid","Balance","Rejected","Accepted"]
    ...
    # ---- STEP 8: Balance_Aging_Summary (robust to empty) ----
    if balance_df.empty:
        ...
        # Ensure all columns present, then fill any NaNs to 0
    ...
    # ---- STEP 8B: Insurance_Totals (Amounts per insurance) ----
    ...
    print("💾 Saving file, please wait... (large files may take up to a minute)")
    wb.save(output_file)
