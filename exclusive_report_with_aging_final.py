...
def parse_args():
    p = argparse.ArgumentParser(
        description="Build Exclusive_Report_with_UnderProcess from an input .xlsx"
    )
...
def compute_measures(df: pd.DataFrame) -> pd.DataFrame:
    # 1) ActivityIns = Net Amount
    # 2) Paid = remit + resub1 + resub2 + resub3 + takeback
    df["Paid"] = df[
        [
            "actRemitInsShare",
    ...
    # Keep negative values from affecting aging/summary logic
    df["UnderProcess"] = pd.to_numeric(df["UnderProcess"], errors="coerce").fillna(0)
    ...
    # 5) If DenialCode is COPY-001 or PRCE-001: move Rejection -> Accepted, then clear Rejection
    ...
    # 6) If ActivityStatus = Submitted and value is in Rejection and UnderProcess is empty/0,
    #    shift Rejection -> UnderProcess
    mask_submitted_back_to_underprocess = (
        activity_status.str.lower().eq("submitted")
        & (df["Rejection"] != 0)
        & (df["UnderProcess"] == 0)
    )
    df.loc[mask_submitted_back_to_underprocess, "UnderProcess"] = df.loc[
        mask_submitted_back_to_underprocess, "Rejection"
    ]
    df.loc[mask_submitted_back_to_underprocess, "Rejection"] = 0.0

    # If same value exists in both UnderProcess and Rejection, remove it from Rejection
    mask_duplicate = (
        (df["UnderProcess"] != 0)
        & (df["Rejection"] != 0)
...
def apply_styling(output_file: str):
    ...
    for ws in wb.worksheets:
        style_headers(ws)
        if ws.title == "UnderProcess_Aging_Summary":
