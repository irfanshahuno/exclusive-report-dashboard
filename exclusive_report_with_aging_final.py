...
def pick_input_file():
    ...
    files = [f for f in glob.glob("*.xlsx") if "Rejection_Report" not in f and "Exclusive_Report_with_Aging" not in f]
    if not files:
        raise FileNotFoundError("❌ No Excel (.xlsx) file found in this folder.")
    return files[0]
...
# ===============================
# Derived measures: Paid / Rejection / Accepted / Balance
# ===============================
df["Paid"] = df[
    ["actRemitInsShare", "actResub1RemitInsShare",
     "actResub2RemitInsShare", "actResub3RemitInsShare",
...
if insurance_col not in df.columns:
    ...
    if insurance_col != "Insurance":
        df["Insurance"] = df[insurance_col]
        insurance_col = "Insurance"

if "Insurance" not in balance_df.columns:
    balance_df["Insurance"] = df["Insurance"]

# ===============================
# Sheet 3: Balance_Aging_Summary
# ===============================
