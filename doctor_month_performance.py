...
import pandas as pd, re, calendar
...
def load_minimal(src) -> pd.DataFrame:
    # src can be a path or file-like (BytesIO/UploadedFile)
...
def build_report(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Item Group"] = (
        ...
        .replace({"nan":"", "NaN":"", "None":""}).where(lambda s: s.ne(""), "Other")
    )
    ...
    visits = (dedup.groupby(["DocName","Year","MonthNum"])["VisitNo"]
    ...
    for g in expected:
        if g not in amount.columns: amount[g] = 0.0
    ...
    ordered = ["Consultation","Medicines","Procedure"] + (["Other"] if "Other" in amount.columns else [])
    amount = amount.reindex(columns=ordered)
    ...
    table["Row_Total"] = table[ordered].sum(axis=1)
    ...
    for doc, block in table.groupby("DocName", sort=False):
        ...
        for c in ordered: total[c] = block[c].sum()
        total["Avg_per_Visit"] = int(round(total["Row_Total"]/total["Visits"])) if total["Visits"]>0 else 0
        block = block[["DocName","Year","MonthNum","Month","Visits"] + ordered + ["Row_Total","Avg_per_Visit"]]
    ...
    return final[["Doctor","Year","MonthNum","Month","Visits"] + ordered + ["Row_Total","Avg_per_Visit"]]
